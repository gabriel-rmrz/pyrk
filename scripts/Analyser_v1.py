...
import uproot
from create_datacard_v1 import create_datacard_signal
from create_datacard_v1 import create_datacard_control
import pickle
...
overflow_underflow = True   #overflow underflow option in plots
fac = False   #factor to the signal yield for the fit
flag = "v1_moreStat" #flag for plot directory and dataset names
...
def preselection(df):
    return df[(df.mu1pt >3) & (df.mu2pt>3) & (df.kpt > 2.5) & (abs(df.mu1eta) < 2.5) & (abs(df.mu2eta) < 2.5) & (abs(df.keta) < 2.5) & (df.Bsvprob > 1e-7) & (abs(df.k_dxy) <0.2) & (abs(df.mu1_dxy)<0.2) & (abs(df.mu2_dxy) < 0.2)   & (df.Bcos2D > 0.95)  & (df.Bmass < 6.3) & (df.k_isPF == 1) & (df.mu1_mediumID == 1) & (df.mu2_mediumID == 1) & (df.Bpt_reco > 15) & (abs(df.mu1_dz-df.mu2_dz)<0.4) & (abs(df.mu1_dz-df.k_dz)<0.4) & (abs(df.mu2_dz-df.k_dz)<0.4) ]
...
if(fit):
    ...
    pf_file=[]
    pf=[]
    df_files = ['hd5_files/OniaX.h5']
    root_files = []

    path = '/pnfs/psi.ch/cms/trivcat/store/user/friti/dataframes_2020Nov19/'

    root_files.append(path + 'BcToXToJpsi_is_jpsi_tau_merged.root')
    root_files.append(path + 'BcToXToJpsi_is_jpsi_mu_merged.root')
    
    for fil in df_files:
        pf_file.append(pd.read_hdf(fil,'pf'))
    for fil in root_files:
        pf_file.append(read_root(fil, 'BTommm'))
    ...
    df_list = []
    df_list.append(read_root(path + 'BcToXToJpsi_is_jpsi_mu_merged.root','BTommm' ))
    df_list.append( read_root(path + 'BcToXToJpsi_is_jpsi_tau_merged.root', 'BTommm'))


    df_list.append( read_root(path + 'BcToXToJpsi_is_chic0_mu_merged.root', 'BTommm'))
    df_list.append( read_root(path + 'BcToXToJpsi_is_chic1_mu_merged.root', 'BTommm'))
    df_list.append( read_root(path + 'BcToXToJpsi_is_chic2_mu_merged.root', 'BTommm'))
    df_list.append( read_root(path + 'BcToXToJpsi_is_hc_mu_merged.root', 'BTommm'))
    #    df_list.append( read_root(path + 'BcToXToJpsi_is_jpsi_3pi_merged.root', 'BTommm'))
    df_list.append( read_root(path + 'BcToXToJpsi_is_jpsi_hc_merged.root', 'BTommm'))
    df_list.append( read_root(path + 'BcToXToJpsi_is_psi2s_mu_merged.root', 'BTommm'))
    df_list.append( read_root(path + 'BcToXToJpsi_is_psi2s_tau_merged.root', 'BTommm'))
    df_list.append( read_root(path + 'data_ptmax_merged.root', 'BTommm'))

    df_list.append( pd.read_hdf('hd5_files/OniaX.h5', 'pf')) #still old
    ...
    for j,sname in zip(range(len(df_list)),['mu','tau','chic0','chic1','chic2','hc_mu','jpsi_hc','psi2s_mu','psi2s_tau','data','oniaAll','comb']):
        print("Processing ",sname)
        df_list[j] = jpsi_branches(df_list[j])
        df_list[j] = preselection_old(df_list[j])
        ...
        if (j<9): # only Bc MC need this weight
    ...
    for df,name in zip(df_list,['mu','tau','chic0','chic1','chic2','hc_mu','jpsi_hc','psi2s_mu','psi2s_tau','data','oniaAll','comb']):
    ...
    sample_all = {}
    sample_names_wOA = ['mu','tau','data','chic0','chic1','chic2','hc_mu','jpsi_hc','psi2s_mu','psi2s_tau','comb','oniaAll']
    sample_names = ['mu','tau','chic0','chic1','chic2','hc_mu','jpsi_hc','psi2s_mu','psi2s_tau','comb','mis_id','data']
...
if(fitPassFail):
        ...
        for sname in sample_all:
            sample_0[sname] = selSoftID(sample_all[sname],0).copy()
        ...
        fromfit = 0.718344
        #sistema
        sample_dic["mu"].norm = 1 * fromfit
        sample_dic["tau"].norm = 0.25 * fromfit
        sample_dic["comb"].norm = norm_fact_onia * 0.34338 * 5.428866 *0.3
        ...
        variables.append(variable("E_mu_star","E_{#mu}^{*}","E_{#mu}^{*}","[GeV]",12,0.2,2.5))
        variables.append(variable("E_mu_canc","E_{#mu}^{#}","E_{#mu}^{#}","[GeV]",10,0.2,4.8))
        variables.append(variable("DR_jpsimu","DR_{J/#psi/#mu}","DR_{J/#psi/#mu}","",10,0,0.7))
        ...
        for var in variables:
            ...
            if (over == True and fac == True):
                ...
                factor = 1
            '''
            makeStack(datah, histos, var, addFileName = "Pass_cut"+cutStr, rootFile = True, ratioPad = False,  path = pathDir)
            ...
            makeStack(datah0, histos0, var, addFileName = "Fail_cut"+cutStr, rootFile = True, ratioPad = False,  path = pathDir)
            ...
            with open(pathDir+"/info.txt","w") as info:
                info.write(
'''
BDT branches:
{bdt_branches}
