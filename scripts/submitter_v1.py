...
for ijob in range(njobs):
    ...
    if not ("BcToXToJpsi") in dataset:
        flauncher.write(
            ...
            xrdcp /scratch/friti/{scratch_dir}/{dat}_UL_{ijob}_ptmax.root root://t3dcachedb.psi.ch:1094///pnfs/psi.ch/cms/trivcat/store/user/friti/{se_dir}/.
            rm /scratch/friti/{scratch_dir}/{dat}_UL_{ijob}_ptmax.root'''.format(dir='/'.join([os.getcwd(), out_dir]), scratch_dir= dataset, cfg='Resonant_Rjpsi_chunk%d.py' %(ijob), option= dataset_opt, dat = dataset,ijob=ijob, se_dir=out_dir)
        )
    else:
        flauncher.write(
            '''#!/bin/bash
    ...
    command_sh_batch = 'sbatch -p wn --account=t3 -o %s/logs/chunk%d.log -e %s/errs/chunk%d.err --job-name=%s  --mem=6GB %s/submitter_chunk%d.sh' %(out_dir, ijob, out_dir, ijob, out_dir, out_dir, ijob)
