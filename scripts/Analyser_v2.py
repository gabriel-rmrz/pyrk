...
flag = "v2" #flag for plot directory and dataset names
...
if(fit):
    ...
    path = '/pnfs/psi.ch/cms/trivcat/store/user/friti/dataframes_2020Nov26/'

    
    root_files.append(path + 'BcToJpsiTauNu_ptmax_merged.root') 
    root_files.append(path + 'BcToJpsiMuNu_ptmax_merged.root')
    root_files.append(path + 'OniaX_ptmax_merged.root') #UL samples
    ...
    df_list = []
    df_list.append(read_root(path + 'BcToJpsiMuNu_ptmax_merged.root','BTommm' ))
    df_list.append( read_root(path + 'BcToJpsiTauNu_ptmax_merged.root', 'BTommm'))
    ...
    df_list.append( read_root(path +'OniaX_ptmax_merged.root', 'BTommm')) 
    df_list.append( read_root(path +'OniaX_ptmax_merged.root', 'BTommm')) 
...
if computeFR:
    ...
    medium1_gaus_int=gaus_fit(histo_medium1,var[0],"medium1_" + flag,pathDir+"FR_plots",[None,5.30,0.06,None,None,None],pol="pol2")
...
if(fitPassFail):
        ...
        sample_dic["tau"].norm = 0.25 * fromfit * 290620./500805. * blind
        sample_dic["comb"].norm = norm_fact_onia * 0.34338 * 5.428866 *0.05
        ...
        variables.append(variable("Bmass","m(3#mu)","m(3#mu)","[GeV]",10,4.5,6))
        ...
        #VERTEX properties
        variables.append(variable("Bcos2D","cos2D","cos2D","",30,0.99,1))
        variables.append(variable("Blxy_sig","#sigma_{L_{xy}}","#sigma_{L_{xy}}","[cm]",15,0,20))
        
        variables.append(variable("Bsvprob","vx_{prob}","vx_{prob}","",40,0,0.02))
        ...
        variables.append(variable("Q_sq","Q^{2}","Q^{2}","[GeV^{2}]",15,3,8))

        variables.append(variable("m_miss_sq","m_{miss}^{2}","m_{miss}^{2}","[GeV^{2}]",10,0,3.5))
        variables.append(variable("E_mu_star","E_{#mu}^{*}","E_{#mu}^{*}","[GeV]",12,1,2.2))
        variables.append(variable("E_mu_canc","E_{#mu}^{#}","E_{#mu}^{#}","[GeV]",10,1.6,4.3))
        ...
        variables.append(variable("mu","#mu_{prob}","#mu_{prob}","",10,0.1,0.8))
